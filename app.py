...
with tab4:
    ...
    if filtered_df['Review Rating'].notnull().sum() == 0:
        ...
        try:
            features = ["Age", "Gender", "Category", "Payment Method"]
            ml_df_encoded = pd.get_dummies(ml_df[features])
            ml_df_encoded["Purchase"] = ml_df["Purchase Amount (USD)"].astype(float)
            X = ml_df_encoded
            ...
            st.success(f"예측 RMSE (낮을수록 정확): {rmse:.2f}")
        except Exception as e:
            st.error(f"머신러닝 실행 중 오류 발생: {e}")
